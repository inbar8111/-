  ...
  return (
    <div className="flex flex-col gap-4" dir="rtl">
      <div className="flex items-center justify-between">
        <h2 className="text-mil-text text-xl font-bold">{missionName}</h2>
        <button
          onClick={() => router.push('/')}
          className="text-mil-muted hover:text-mil-text text-sm transition-colors"
        >
          ← חזרה
        </button>
      </div>
      ...
      <div className="bg-mil-card border border-mil-border rounded-lg overflow-hidden">
        <div className="flex items-center gap-2 px-3 py-2 bg-mil-surface border-b border-mil-border text-mil-muted text-xs font-medium" dir="rtl">
        ...
        <div className="flex flex-col gap-1 p-3">
          ...
          {rows.length === 0 && (
            <p className="text-mil-muted text-sm text-center py-4">אין שורות. לחץ "הוסף שורה" להוספת חיילים.</p>
          )}
        </div>
      ...
      <div className="bg-mil-card border border-mil-border rounded-lg p-4 flex flex-col gap-3">
        <div className="flex items-center gap-3">
          <label className="text-mil-text text-sm font-medium whitespace-nowrap">שעת עליה למשימה:</label>
          <input
            type="datetime-local"
            value={startTime}
            onChange={e => setStartTime(e.target.value)}
            className="bg-mil-surface border border-mil-border text-mil-text rounded px-3 py-1.5 text-sm focus:outline-none focus:border-mil-primary flex-1 max-w-xs"
          />
        </div>
        ...
        <div className="flex items-center justify-between">
          <span className="text-mil-muted text-sm">
            {validCount} חיילים תקינים מתוך {rows.length} שורות
          </span>
          <button
            onClick={handleSave}
